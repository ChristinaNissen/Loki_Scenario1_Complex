...
const VisualSelectionPicture = () => {
  ...
  // Close modal if all vote identifiers are removed
  useEffect(() => {
  ...
  const confirmSelection = async () => {
    // Get base names for selected vote identifiers
    const selectedBaseNames = selected.map(imgSrc => {
      if (!imgSrc || typeof imgSrc !== 'string') return '';
      return getBaseName(imgSrc);
    }).filter(Boolean);

    // Handle visualRepresentation - for vote identifiers, look for image_visual key
    let visualBaseName = '';
    if (visualRepresentation && typeof visualRepresentation === "object") {
      // Check for image_visual key specifically for vote identifiers
